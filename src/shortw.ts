...
// TODO: OptsOther der: false => format: 'der' or something
// TODO: sign(canonical: true) vs verify(strict: true) naming?
// TODO: ban randomBytes reassign
 ...
 */
function sqrtMod(number: bigint, modulo: bigint): bigint {
  const n = number;
  const P = modulo;
  ...
  // P = 3 (mod 4)
  // sqrt n = n^((P+1)/4)
  if (P % _4n === _3n) return powMod(n, p1div4, P);
  // P = 5 (mod 8)
  if (P % _8n === _5n) {
    const n2 = mod(n * _2n, P);
    const v = powMod(n2, (P - _5n) / _8n, P);
    const nv = mod(n * v, P);
  ...
  if (s === 1) return powMod(n, p1div4, P);
...
export function weierstrass(
  ...
  const sqrtModCurve = overrides.sqrtMod || sqrtMod;
  ...
  class JacobianPoint implements JacobianPointType {
    ...
    toAffine(invZ?: bigint): Point {
      ...
      if (invZ == null) invZ = is0 ? _8n : invert(z, CURVE.P);
      const iz1 = invZ;
  ...
  function recoverPublicKey(
    ...
    const instance = normalizeSignature(signature);
